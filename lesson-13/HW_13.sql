...
insert into Employees values ( 100,trim('Steven '),trim('King '), trim('SKING '),trim('515.123.4567 '),'1987-06-17',trim('AD_PRES '),24000.00,0.00, 0,90),
...
Select  year1,max(maxx)from 
(Select Year1,(max1) as maxx from TestMax
union all
select year1,(max2) from TestMax
union all
select year1,(max2) from TestMax) as maa
group by Year1
...
Select coalesce(null,null,null,'Person') 
--Medium task
--1
select * from Students
select Substring(FullName,1,CHARINDEX(' ',FullName)-1) as FirstName,
right(FullName,len(FullName)-CHARINDEX(' ',FullName)) as Middlename,
Substring (right(FullName,len(FullName)-CHARINDEX(' ',FullName)),1,CHARINDEX(' ',right(FullName,len(FullName)-CHARINDEX(' ',FullName)))) as LastName
...
SELECT 
    ...
    CAST(COUNT(CASE WHEN DATEDIFF(YEAR, HIRE_DATE, GETDATE()) > 3 THEN 1 END) * 100.0 
...
--1
--2
Select StudentID,FullName,sum(grade) over(order by studentID) from Students
--3
...
WHERE EXISTS (
    ...
    FROM Student AS S2
    WHERE 
        S2.StudentName <> Student.StudentName
