...
DROP TABLE IF EXISTS Products; DROP TABLE IF EXISTS Suppliers;
...
--3
Select  SupplierName,ProductName from Products join Suppliers on Products.SupplierID=Suppliers.SupplierID
--4
Select FirstName+LastName as Customer_Name,OrderID from orders join Customers on Orders.CustomerID=Customers.CustomerID
--5
Select * from Courses cross join Students
--6
select ProductName,orders.* from Products join Orders on Products.ProductID=Orders.ProductID
--7
Select * from Departments join Employees on Departments.DepartmentID=Employees.DepartmentID
--8
select name,CourseID from Students join Enrollments on Students.StudentID=Enrollments.StudentID
--9
select * from Payments join Orders on Payments.OrderID=Orders.OrderID
--10
select * from orders cross join Products where Products.Price>100
--11
select * from Employees cross join Departments where Employees.DepartmentID!=Departments.DepartmentID
...
--14
select name,CourseName from Courses join Enrollments on Courses.CourseID=Enrollments.EnrollmentID
join Students on Students.StudentID=Enrollments.EnrollmentID
--15
Select ProductName,SupplierName from Products join Suppliers on Products.SupplierID=Suppliers.SupplierID
where SupplierName like '%Tech%'
--16
Select * from Orders join Payments on Orders.OrderID=Payments.OrderID
where Payments.Amount<Orders.TotalAmount
--17
Select a.name,a.salary,b.Name manager_name,b.salary as manager_salary  from Employees a join Employees b on a.ManagerID=b.EmployeeID
where a.Salary>b.Salary
--18
Select * from Products join Categories on Products.Category=Categories.CategoryName
...
--24
Select * from Employees a join Employees b on a.EmployeeID=b.ManagerID
where b.Salary<a.Salary
--25
select * from orders o join Payments p on o.OrderID=p.OrderID right join Customers c on c.CustomerID=o.CustomerID
