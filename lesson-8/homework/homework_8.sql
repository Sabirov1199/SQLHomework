...
Except
select CustomerID from Invoices where year(InvoiceDate)='2023'
--11
...
--14
Select ProductName,Price, case when Price<100 then 'Low'
when price >500 then 'High' else 'Mid'   end as pricegroup from Products
--15
Select distinct City from Customers
order by city asc
--16
Select ProductID,count(saleID) from Sales
group by ProductID
--17
Select ProductName from Products
...
--19
Select top 3 CustomerID,max(TotalAmount)  from Invoices
group by CustomerID
--20
...
--21
Select ProductName,count(saleID) from Products p join sales s on p.ProductID=s.ProductID
group by ProductName
--22
Select top 5 ProductID,max(Quantity)  from Orders
group by ProductID